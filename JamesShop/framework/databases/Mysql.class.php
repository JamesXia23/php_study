...
class Mysql{
	...
	public function __construct($config = array()){
		...
		$this->conn = mysql_connect("$host:$port",$user,$password) or die('数据库连接错误');//or前面为假就会执行后面
	...
	public function query($sql){
		...
		if ($GLOBALS['config']['debug']) {
			$str = "[". date("Y-m-d H:i:s") . "] ". $sql . PHP_EOL;//PHP_EOL:php的换行符,跨平台
	...
	public function getRow($sql){
		if ($result = $this->query($sql)) {
			$row = mysql_fetch_assoc($result);//获取结果集中的第一条记录,类型为一维数组
			return $row;
		} else {
			return false;
		}
	...
	public function getCol($sql){
		...
		while ($row = mysql_fetch_row($result)) {
			$list[] = $row[0];//$list[]这样每次赋值,指针就会下移,即下标:0,1,2,3..
		}
		return $list;
	}
